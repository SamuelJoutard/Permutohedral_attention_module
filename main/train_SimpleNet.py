...
if __name__=="__main__":
    ...
    for k in range(5):
        ...
        non_local = False
        dilated = False
        save_dir = "3D_Tests/final_results/SimpleNet_fold" + str(k+1)
        ...
        non_local = True
        dilated = False
        save_dir = "3D_Tests/final_results/SimpleNet_nl_fold" + str(k+1)
        if not os.path.isdir(save_dir):
            os.mkdir(save_dir)
        model = SimpleNet(non_local=non_local, dilated=dilated)
        if load_:
        ...
        non_local = False
        dilated = True
        save_dir = "3D_Tests/final_results/SimpleNet_dil_fold" + str(k+1)
        if not os.path.isdir(save_dir):
            os.mkdir(save_dir)
        model = SimpleNet(non_local=non_local, dilated=dilated)
        if load_:
        ...
        save_dir = "3D_Tests/final_results/SimpleNet_nl_dil_fold" + str(k+1)
        if not os.path.isdir(save_dir):
            os.mkdir(save_dir)
        model = DilFCN_PAM()
        if load_:
